...
export function buildSystemPrompt() {
  return [
    ...
    '- Always respond **only** with JSON (no prose) describing what is visually present.',
    '- Use keys: subject.description, subject.physique, subject.expression, clothing.type, clothing.style, clothing.color,',
    ...
    '- Keep tone factual. Never invent scenery or attributes that are not clearly visible. If something is unclear, return an',
    '  empty string for that field.',
    '- If the image payload is missing or unreadable, reply with a JSON object that sets every field to an empty string and adds',
